...
class DeviceSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "Device actor" must {
    ...
    "return temperature reading even if device stops after answering" in {
      ...
      requester.expectMessage(RespondAllTemperatures(requestId = 1, temperatures = Map("device1" -> Temperature(1.0), "device2" -> Temperature(2.0))))
    }
    ...
    "return DeviceTimedOut if device does not answer in time" in {
      ...
      val queryActor = spawn(DeviceGroupQuery(deviceIdToActor, requestId = 1, requester = requester.ref, timeout = 200.millis))
      ...
      requester.expectMessage(RespondAllTemperatures(requestId = 1, temperatures = Map("device1" -> Temperature(1.0), "device2" -> DeviceTimedOut)))
    }
